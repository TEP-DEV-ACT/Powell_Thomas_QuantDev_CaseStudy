...
from tracker.config import ANTHROPIC_API_KEY
...
def test_structured_question_uses_get_fundamentals():
    ...
    result = ask("What were NVDA's revenue and net income for the last three fiscal years?")
...
def test_cross_source_question_uses_get_valuation():
    ...
    result = ask("What is AAPL's trailing P/E right now?")
...
def test_filing_text_question_uses_search_filings():
    ...
    result = ask("What new risk factors did NVDA add in its latest 10-K versus the prior year?")
...
def test_cross_modal_question_uses_both_numbers_and_text():
    ...
    result = ask("How did MSFT's revenue grow last year, and what did management attribute it to?")
